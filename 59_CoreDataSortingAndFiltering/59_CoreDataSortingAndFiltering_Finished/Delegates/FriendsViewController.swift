...
class FriendsViewController: UIViewController {
    ...
    func fetchFriendsWithFilter(filter: String) {
        let fetchRequest: NSFetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "FriendEntity")
        fetchRequest.predicate = NSPredicate(format: "isInvited.invited == false && name CONTAINS[c] %@", filter)
        fetchRequest.sortDescriptors = [NSSortDescriptor(key: "name", ascending: true)];
...
extension FriendsViewController: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        if searchBar.text != nil {
            self.fetchFriendsWithFilter(filter: searchBar.text!)
            self.tableView.reloadData()
        }
    }
