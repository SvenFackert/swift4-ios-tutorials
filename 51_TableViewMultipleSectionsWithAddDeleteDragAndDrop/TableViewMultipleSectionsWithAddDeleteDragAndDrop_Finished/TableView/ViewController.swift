...
class ViewController: UIViewController {
    ...
    override func setEditing(_ editing: Bool, animated: Bool) {
        ...
        if editing {
            ...
            self.tableView!.insertSections(IndexSet(integer: self.people.count), with: .automatic)
        }
        else {
            self.tableView.setEditing(false, animated: true)
            self.tableView!.deleteSections(IndexSet(integer: self.people.count), with: .automatic)
        }
    }
...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if self.isEditing == true && section == self.people.count {
            return 1
        }
        else {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == self.people.count && self.isEditing == true {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            self.people[indexPath.section].remove(at: indexPath.row)
            self.tableView.deleteRows(at: [indexPath], with: .automatic)
        }
        else if editingStyle == .insert {
            people[0].append(Person(name: "Chuck Norris", personImageName: "chucknorris", flagImageName: "usa"))
            self.tableView.insertRows(at: [IndexPath(row:self.people[0].count - 1, section: 0)], with: .automatic)
        }
    }

    
    func tableView(_ tableView: UITableView, canMoveRowAt indexPath: IndexPath) -> Bool {
        if indexPath.section == self.people.count {
            return false
        }
        else {
            return true
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        if sourceIndexPath.row < self.people[sourceIndexPath.section].count && destinationIndexPath.row <= self.people[destinationIndexPath.section].count {
            let person: Person? = self.people[sourceIndexPath.section][sourceIndexPath.row]
            self.people[sourceIndexPath.section].remove(at: sourceIndexPath.row)
            if person != nil {
                self.people[destinationIndexPath.section].insert(person!, at: destinationIndexPath.row)
            }
        }
    }
}
...
extension ViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, targetIndexPathForMoveFromRowAt sourceIndexPath: IndexPath, toProposedIndexPath proposedDestinationIndexPath: IndexPath) -> IndexPath {
        if proposedDestinationIndexPath.section >= self.people.count {
